...
for xd in range(10000):
	...
	ugen.append(uaku2)
for x in range(10):
	a='Mozilla/5.0 (Linux; Android 12; SAMSUNG SM-G780G'
	...
	i='AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/16.0 Chrome/92.0.4515.166'
...
def uaku():
	try:
		...
		a=requests.get('https://github.com/muniroh96/fbhend/blob/main/bbnew.txt').text
		ua=open('.bbnew.txt','w')
...
╚██████╔╝██║░░██║██║░╚═╝░██║
░╚═════╝░╚═╝░░╚═╝╚═╝░░░░░╚═╝
			{m}•{k}•{h}•{sir} Author : Hendry {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
if __name__=='__main__':
	...
	alvino_xy(f'\n\t{x}——> {h}Gunakan Script Ini dengan BRUTAL\n\t{x}——> {h}Hendgam Sehat Selalu Yah\n\t{x}——> {h}Semoga Di Mudahkan Rezekinya Amin\n\t{x}——> {h}Semoga Harimu mengontolkan{x}')
	time.sleep(3)
	login()
